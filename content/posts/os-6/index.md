...
draft = false
title = '第六章 并发程序设计'
summary = "本章介绍了并发程序设计的基本概念、并发进程的制约关系、临界区管理、管程等。重点发是理解并发程序设计和普通程序思路上的不同，以及使用信号量和管程来解决并发程序设计中的问题。"
tags = ["笔记", "OS", "操作系统"]
...
管程是一种特殊的软件模块，它由一组只属于自己的共享变量、这些变量的说明，以及所有能访问这些变量的过程/方法/函数组成。管程把所有对共享资源的访问都集中到一个地方（即管程内部），由系统自动保证互斥和同步。这样不仅更安全，也更方便用高级语言来编写并发程序。
...
每个过程都可以访问管程的局部变量和条件变量。

#### 执行模型

![](hoare_monitor_1.png)

首先，从图的顶部“入口”处，**等待调用的进程队列**表示有多个进程想要进入管程，但由于管程具有互斥性，同一时刻只能有一个进程进入管程内部执行。其他进程则在入口处排队等待。

进程进入管程后，可以访问管程内部的**局部数据**（即只属于管程的共享变量）、**条件变量**（用于同步）、以及管程定义的各个**过程**（如过程1到过程k），这些过程是对外开放的接口，外部进程只能通过调用这些过程来操作管程的局部数据，不能直接访问数据本身。

在管程内部执行过程中，如果进程遇到某些条件不满足（比如资源不足），就会在相应的**条件变量**（如`condition c1`到`condition cn`）上执行`wait(c1)`、`wait(cn)`等操作。此时，进程会被挂起，进入**管程等待区域**，在对应的条件变量队列中等待条件满足。这样可以防止进程在条件不满足时占用管程，释放管程让其他进程有机会进入。

当管程内的某个过程执行到可以唤醒等待进程的时候，会调用`signal`操作。`signal`会把在某个条件变量上等待的进程唤醒，并把它放入**next queue**（下一个等待队列）。next queue的作用是保证被唤醒的进程能够优先获得管程的执行权，防止多个进程同时进入管程，破坏互斥性。

被唤醒的进程从next queue进入管程，继续执行剩下的操作。所有进程在管程内执行完毕后，从“出口”离开管程，释放对管程的占用，允许下一个等待的进程进入。

#### 重要属性  

- 共享性：多个进程可以通过调用管程来访问同一组资源。  
- 安全性：进程不能随意访问管程外的共享变量，必须通过管程提供的过程来操作。  
- 互斥性：同一时刻，最多只有一个进程能在管程内执行，自动实现了互斥。

#### signal操作的细节问题

当一个进程在管程内执行`signal`操作唤醒另一个等待进程时，可能会出现两个进程同时在管程内的情况。为了解决这个问题，有两种主要做法：  

1. 执行`signal`的进程主动让出管程，等待被唤醒的进程执行完毕后再继续（Hoare方案）。  
2. 被唤醒的进程等到执行`signal`的进程离开管程后再进入（Hansen方案，或规定signal必须是过程的最后一步）。

### 4.2 管程的实现：霍尔管程

在Hoare管程中，每个管程都配有一个名为`mutex`的信号量（初值为1），用于保证管程的互斥性。任何进程只要想调用管程中的过程，都必须先执行`P(mutex)`，即申请进入管程。如果管程已经被其他进程占用，当前进程就会被阻塞在这里。进程在退出管程时，需要判断是否有进程在`next`信号量上等待（即`next_count > 0`）。如果有，就用`V(next)`唤醒一个等待的进程；如果没有，就用`V(mutex)`释放管程，让其他进程可以进入。

在管程内部，如果进程因为条件不满足需要等待（即执行`wait`操作），也必须先执行`V(mutex)`，这样可以让其他进程有机会进入管程，避免所有进程都被阻塞在管程外，导致资源无法被释放。

每个管程还引入了`next`信号量（初值为0），用于协调signal操作后的进程切换。当一个进程在管程内执行`signal`操作时，它会用`P(next)`主动阻塞自己，直到被唤醒。只有等被唤醒的进程执行完并退出管程后，signal进程才会被唤醒继续执行。这样可以保证同一时刻只有一个进程在管程内，严格遵守Hoare管程的语义。

`next_count`是一个计数器，用来记录有多少进程在`next`信号量上等待。进程在退出管程前要检查`next_count`，如果有等待的进程，就用`V(next)`唤醒它，否则用`V(mutex)`释放管程。

对于每个条件变量（如`x`），都配有一个信号量`x-sem`（初值为0）和一个计数器`x-count`（初值为0）。当进程在管程内因为某个条件不满足而需要等待时，会执行`P(x-sem)`，自己阻塞在该条件变量上，并将`x-count`加1。这样，系统就能知道有多少进程在等待这个条件。

当有进程执行`signal(x)`时，会用`V(x-sem)`唤醒在该条件变量上等待的进程中的一个。这样可以保证被唤醒的进程能立即获得管程的执行权，而不会被其他新来的进程抢先进入管程。

```C
typedef struct interface_module {
  semaphore mutex;
  semaphore next;
  int next_count;
} IM;
...
```C
void wait(semaphore &x_sem, int x_count, IM &im) {
  x_count++;
  if (x_count > 0) {
    V(im.next);
  } else {
...
void signal(semaphore &x_sem, int x_count, IM &im) {
  if (x_count > 0) {
    im.next_count++;
    V(x_sem);
    P(im.next);
...
用霍尔管程实现之前的信号量时的各种问题。问题思路和之前都差不多，这里摆了，只给出伪代码。
